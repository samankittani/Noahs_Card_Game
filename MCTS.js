...
class actionClass {
  ...
  update nWins, nPick, and UCB. It is the expection of the paths.*/
    updateValues = function() {
        if (this.node.rolloutType === ROLLOUTTYPES.sBiasRand) {
            this.maxWiden = this.node.widenParam[0] * (this.nPicks ** this.node.widenParam[1]);
            this.expUtility = this.updateUtilityRand();
        } else {
            throw new Error(`Error: rolloutType ${this.node.rolloutType} not implemented`)
        }
    }
    ;
    /* 
  update the actions UCB value */
    updateUCB1 = function() {
        if (this.nPicks !== 0) {
            this.UCB = this.expUtility + this.node.exploreParam * Math.sqrt(Math.log(this.node.nSamples) / this.nPicks);
        }
    }
    /* 
    ...
    getNextNode = function() {
        if (this.paths.length < this.maxWiden) {
            ...
            if (i !== -1) {
                ...
                let newNode = new MCTNodeClass(nextState,this.node.exploreParam,this.node.widenParam,this.node.rolloutType,this);
            ...
            for (let i = 0; i < this.paths.length; i++) {
                if (randInt < this.paths[i].nSamples) {
                    return this.paths[i];
                }
                randInt = -this.paths[i].nSamples;
            }
        }
  ...
  attempt to refill hand of active player*/
    refreshHand = function(nextState) {
        ...
        while (nextState.myHand.length < 3 && (nextState.myD.length || nextState.myDis.length)) {
    ...
    performAction = function(nextState) {
        if (this.action === ACTIONS.endTurn) {/* do nothing*/
            ...
            if (card.wPoints === 15 && nextState.shopFD.length !== 0) {
                let randShopInd = getRandomInt(0, nextState.shopFD - 1);
                nextState.shop.push(nextState.shopFD.splice(randShopInd, 1)[0])
            } else if (nextState.shopD.length !== 0) {
                let randShopInd = getRandomInt(0, nextState.shopD - 1);
...
***********************************************************************************************************/
class MCTNodeClass {
    constructor(state, exploreParam, widenParam, rolloutType, parentAction=null) {
  ...
  used when backpropogating*/
    updateValues = function() {
        ...
        for (const action of this.actions) {
            if (action.nPicks !== 0) {
                action.updateUCB1();
                this.utility += action.expUtility;
            }
        }
    ...
    /* 
  sum the utilities of all actions of node. = utility */
    updateUtil = function() {
        this.utility = 0;
        for (const action of this.actions) {
            if (action.nPicks !== 0) {
  ...
  populate all possible actions for node*/
    updateActions = function() {
        ...
        let sum = 0;
        let black = false, red = false;
        console.log()
        for (let i = 0; i < this.state.myHand.length; i++) {
            sum += this.state.myHand[i].number;
            if (this.state.myHand[i].wPoints === 15 && this.state.myHand[i].suit === SUITS.club && !black) {
                this.actions.push(new actionClass(ACTIONS.playBlack,-1,this));
                black = true;
            }
            if (this.state.myHand[i].wPoints === 15 && this.state.myHand[i].suit === SUITS.spade && !red && this.state.oppHand.length !== 0) {
...
class MCTSClass {
  ...
  initialize root node to the known state of the game*/
    init = function() {
        ...
        // create root node
        this.root = new MCTNodeClass(initState,this.exploreParam,this.widenParam,this.rolloutType);
    }
    /* 
  Run MCTS. 
  does one iteration*/
  ...
  it is a leaf node and needs a rollout*/
    getBestLeaf = function() {
        ...
        while (node.nSamples !== 0 || node === this.root) {
            let bestActionInd = node.getNextActionInd();
            console.log(node.actions[bestActionInd])
  ...
  */
  ...
  When we reach the root, we update GLOBAL_ITER */
    backProp = function(res, leaf) {
        leaf = this.setLeafUtil(res, leaf);
        let node = leaf;
    ...
    /* 
  set initial utility for leaf */
    setLeafUtil = function(res, leaf) {
        if (res.outcome === OUTCOMES.rWin && leaf.state.human === true) {
            leaf.utility = 1;
        } else if (res.outcome === OUTCOMES.rWin && leaf.state.human === false) {
            leaf.utility = -1;
        } else if (res.outcome === OUTCOMES.rLost && leaf.state.human === true) {
            leaf.utility = -1;
        } else if (res.outcome === OUTCOMES.rLost && leaf.state.human === false) {
            leaf.utility = 1;
        } else {
            leaf.utility = 0;
        }
  ...
  state must be a deep copy.*/
    rolloutBRand = function(state) {
        ...
        while (rolloutNode.state.shopFD.length !== 0 || rolloutNode.state.shopD.length !== 0) {
...
const MCTS = new MCTSClass(10000,[0.5, 1],2,ROLLOUTTYPES.sBiasRand);
